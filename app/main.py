...
def image_to_byte_array(image: Image, image_format: str) -> str:
  img_buff = io.BytesIO()
  image.save(img_buff, format=image_format)
  return base64.b64encode(img_buff.getvalue()).decode('utf-8')


@app.post('/predict/count')
def predict_count(files: List[UploadFile] = File(...)):
    """
...
# @app.post('/predict/image', responses={200: {'content': {"image/png": {}, "image/jpeg": {}}}}, response_class=Response)
@app.post('/predict/image')
def predict_image(files: List[UploadFile] = File(...)):
    """
    Animal Prediction in image.
    ...
    images = model(images)
    images.render()
    images = [Image.fromarray(im) for im in images.ims]
    return [image_to_byte_array(image, 'JPEG') for image in images]#or PNG